...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── SCAN AREA (KIRIM) ── */
    .scan-area { flex: 1; display: flex; flex-direction: column; padding: 12px 14px; gap: 10px; overflow: hidden; }
    ...
    #scanInput {
        width: 100%; padding: 12px 14px; background: rgba(59,130,246,0.1);
        border: 2px solid rgba(59,130,246,0.35); border-radius: 8px; color: #e2e8f0;
        font-size: 16px; text-align: center; font-weight: 600; letter-spacing: 1px; outline: none;
    }
    #scanInput:disabled { opacity: 0.35; cursor: not-allowed; }
    
    .counter-row { display: flex; justify-content: space-between; flex-shrink: 0; }
    .counter-box {
        background: linear-gradient(135deg, rgba(59,130,246,0.15), rgba(99,102,241,0.1));
        border: 1px solid rgba(100,200,255,0.25); border-radius: 10px; padding: 10px;
        text-align: center; flex: 1;
    }
    .counter-label { font-size: 10px; color: #64748b; text-transform: uppercase; }
    .counter-value { font-size: 32px; font-weight: 900; color: #64c8ff; }

    .scan-list { flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 6px; }
    .scan-item {
        background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.2);
        border-radius: 8px; padding: 9px 12px; display: flex; justify-content: space-between; align-items: center;
    }

    /* ── BUTTONS ── */
    .mulai-footer { padding: 10px 14px; background: rgba(11,18,32,0.98); border-top: 1px solid rgba(59,130,246,0.15); flex-shrink: 0; }
    .mulai-footer.hidden { display: none; }
    .btn-start {
        width: 100%; padding: 14px; background: linear-gradient(135deg, #3b82f6, #6366f1);
        border: none; border-radius: 12px; color: white; font-size: 15px; font-weight: 800; cursor: pointer;
    }
    .btn-start:disabled { background: rgba(255,255,255,0.06); color: #475569; cursor: not-allowed; }
    .center-actions { display: none; gap: 12px; padding: 8px 14px; flex-shrink: 0; }
    .center-actions.visible { display: flex; }
    .btn-center { flex: 1; padding: 12px; border-radius: 12px; border: none; font-weight: 800; font-size: 15px; cursor: pointer; }
    .btn-cancel { background: rgba(239,68,68,0.13); color: #f87171; border: 1px solid rgba(239,68,68,0.35); }
    ...
    .terima-detail {
        ...
        border-radius: 12px; padding: 12px; margin-top: 10px; display: none;
    }
    .terima-detail.visible { display: block; }
    .td-row { display: flex; justify-content: space-between; margin-bottom: 6px; font-size: 12px; }
    .td-label { color: #94a3b8; }
    .td-val { font-weight: 600; color: #e2e8f0; }

    .toast {
        position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%);
        padding: 11px 18px; border-radius: 10px; font-size: 13px; font-weight: 600; z-index: 999;
    }
    .toast.error { background: #fee2e2; color: #ef4444; }
    .toast.success { background: #dcfce7; color: #22c55e; }

    .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.7); display: flex; align-items: center; justify-content: center; z-index: 500; padding: 20px; }
    .modal-overlay.hidden { display: none; }
    .modal-box { background: #111827; border: 1px solid rgba(59,130,246,0.25); border-radius: 16px; padding: 20px; width: 100%; max-width: 340px; }
    ...
    <div class="tab-content active" id="contentKirim">
        <div class="setup-panel" id="setupKirim">
            <div class="setup-row">
                ...
                <input list="driverList" class="setup-input" id="supirInput" placeholder="Ketik/pilih supir..." autocomplete="off">
            ...
            <div class="setup-row" style="margin-bottom:0">
                ...
                <input type="text" class="setup-input" id="mobilKirimInput" placeholder="Arahkan scanner ke kendaraan...">
            </div>
        </div>
    ...
    <div class="tab-content" id="contentTerima">
        <div class="setup-panel">
            ...
            <div class="terima-detail" id="terimaDetail">
                <div class="td-row"><span class="td-label">Status:</span> <span class="td-val" style="color:#4ade80;">Ditemukan</span></div>
                ...
                <button class="btn-start" id="btnProsesTerima" style="margin-top:10px;" disabled>SELESAIKAN PENERIMAAN</button>
            </div>
        </div>
    </div>
</div>
...
document.getElementById('btnConfirmOperator').addEventListener('click', () => {
    ...
    if (!sel.value) { showToast('Pilih operator!'); return; }
...
['supirInput', 'lokasiAsalInput', 'mobilKirimInput'].forEach(id => {
    document.getElementById(id).addEventListener('input', checkStartKirim);
});
...
document.getElementById('scanInput').addEventListener('keypress', async (e) => {
    ...
    const kode = e.target.value.trim(); e.target.value = '';
    if (!kode) return;
    ...
    try {
        ...
        const data = await res.json();
        if (!data.success && data.duplicate) { showToast('Rak sudah discan!'); return; }
        if (!data.success) throw new Error(data.error);
        ...
        div.innerHTML = `<span>${kode}</span> <span style="font-size:10px;color:#94a3b8">${data.waktu_scan}</span>`;
